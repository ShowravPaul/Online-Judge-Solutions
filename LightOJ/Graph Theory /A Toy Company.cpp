...
void bfs(string x) {
   ...
   while(q.sz) {
    string node = q.front().ff;
    ll cost = q.front().ss;
    q.pop();

    if(t == node) {ans = min(cost,ans); continue;}

    if(forbid.count(node)) continue;

    if(vis[node[0]-'a'][node[1]-'a'][node[2]-'a']) {/*debug(node);*/ continue;}

    vis[node[0]-'a'][node[1]-'a'][node[2]-'a'] = 1;
...
void solve()
{
    ...
    if(forbid.count(t) || forbid.count(s)) ans = -1;
    else bfs(s);
