...
ll ans = 0;
void dfs(ll s, ll d, ll state) {
    vis[s] = 1;

    if(state == 1) {
        if(d > mxdis) mxdisnode = s, mxdis = d;
    }
    else if(state == 2) {
        if(d > ans) ans = d;
    }
...
void solve()
{
     ...
     // 2 ta dfs, ekta just 0 theke max dis er node ta ber korchi mane ekta corner e gechi
     //er pore main dfs run korchi
    ll n; cin >> n;
    ...
    // kon node theke start korbo ei jnno
    dfs(0, 0, 1);
    // debug(mxdisnode)
