...
vector< pair< pair<ll, ll>, ll > >v[N]; // node, weight and directed or not
 // vis lagbe na
set<pair<ll, ll>> SET;
ll cost = 0;

void dfs(ll s, ll d, ll parent) {
    cost = max(cost, d);
    ...
    for(auto t: v[s]) {
        ...
        pll p = {s, nextNode};
        if(SET.count(p)) continue; // kondike gelam mark rakhtechi

        SET.in(p);
        if(directed) dfs(nextNode, d, s);
        else dfs(nextNode, d + w, s);

        break; // just ekdike jabe
    }
}

void solve()
{
    // jehetu circle tai full right e ekbar jabo and full left e ekbar jabo jeta te cost kom lage oitai ans
    ll n; cin >> n;
    ...
    dfs(1, 0, -1); // ekdike gelam
    ll ans1 = cost;
    cost = 0;

    dfs(1, 0, -1); // onnodike gelam
    ll ans2 = cost;
