...
vector<ll>findPrimeFactors(ll n)
{
    ...
    if (n > 2) s.in(n);
    for(auto t : s) primeFactors.pb(t);
    return primeFactors;
}
...
void BFS(ll s) {
   ...
   while(q.sz) {
    ...
    if(u == m) {ans = min(ans, dis[u]); return;}

    for(auto t: PF[u]) { //PF[u] preprocess kora ache
        if(t != u and t + u <= m and !vis[t + u]) {
            vis[t + u] = 1;
            dis[t + u] = dis[u] + 1;
            q.push(t + u);
        }
